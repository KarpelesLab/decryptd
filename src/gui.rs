...
//! line we refresh once a second, and a Quit item that ends the process.
...
/// the loop can refresh its text), and the Quit item's id.
fn build_tray(status: &Status) -> Result<(TrayIcon, MenuItem, MenuId)> {
    let menu = Menu::new();
    ...
    Ok((tray, status_item, quit.id().clone()))
}
...
fn tick(status: &Status, status_item: &MenuItem, quit_id: &MenuId) {
...
fn run_tray(status: Status) -> Result<()> {
    ...
    let (_tray, status_item, quit_id) = build_tray(&status)?;

    glib::timeout_add_seconds_local(1, move || {
        tick(&status, &status_item, &quit_id);
...
fn run_tray(status: Status) -> Result<()> {
    ...
    loop {
        ...
        // ...then refresh status / handle Quit and idle briefly.
        tick(&status, &status_item, &quit_id);
